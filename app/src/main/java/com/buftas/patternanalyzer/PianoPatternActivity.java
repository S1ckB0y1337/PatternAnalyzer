...
public class PianoPatternActivity extends AppCompatActivity {

    private TextView attemptCounter, progressCounter;
    private EditText username_field;
    private String currentUsernameInUse = "";
    private Button[] pianoButtonArray;
    private Button start, confirm, cancel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_piano_pattern);
        attemptCounter = findViewById(R.id.pianoAttemptCounter);
        username_field = findViewById(R.id.pianoPatternUsername);
        progressCounter = findViewById(R.id.pianoProgressCounter);
        initializeButtons();
    ...
    private void initializeButtons() {
        ...
        start = findViewById(R.id.pianoPatternStartButton);
        start.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                InputMethodManager inputManager = (InputMethodManager)
                        getSystemService(PianoPatternActivity.this.INPUT_METHOD_SERVICE);
        ...
        confirm = findViewById(R.id.pianoPatternConfirmButton);
        confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (patternStep < 4) {
                    ...
                    //Now i calculate the Pattern Length in pixels by calculating and summing the distance between each fo the points an even occurred
                    ...
                    if (attemptNo == 11) {
                        ...
                        //Update the database about the user
                        setUserDoneWithPianoPattern();
        ...
        cancel = findViewById(R.id.pianoPatternCancelButton);
        ...
        defaultValues = new int[13];
        for (int i = 0; i < 13; i++) {
            defaultValues[i] = buttonCounter;
            buttonCounter++;
        }

        buttonCounter = 1;
        //Initialize Color Pattern Buttons
        pianoButtonArray = new Button[13];
        for (int i = 0; i < 13; i++) {
            String buttonID = "pianobutton" + buttonCounter;
            int resID = getResources().getIdentifier(buttonID, "id", getPackageName());
            pianoButtonArray[i] = findViewById(resID);
            final Button tempButton = findViewById(resID);
            //Get the original color of the button for later re-paint use
            final int defaultValue = defaultValues[i];
            final RawPatternEntry tempEntry = new RawPatternEntry();
            pianoButtonArray[i].setOnTouchListener(new View.OnTouchListener() {
                @Override
                public boolean onTouch(View v, MotionEvent event) {
                    switch (event.getAction()) {
                        case MotionEvent.ACTION_DOWN:
                            ...
                            if (patternStep == 4) {
                                showMessage("Error!", "Pattern can have max 4 entries!" + "Current pattern is: " + Arrays.toString(pattern));
                                return true;
                            } else {
    ...
    private boolean confirmUser() {
        ...
        DatabaseConnectionHelper db_connection = new DatabaseConnectionHelper(PianoPatternActivity.this);
        ...
        if (!userExists(username, database)) {
            ...
            return false;
        } else if (userExists(username, database) && !isUserDoneWithPianoPattern(username, database)) {
    ...
    private boolean isUserDoneWithPianoPattern(String username, SQLiteDatabase database) {
        Cursor c = database.query(DatabaseSchema.UsernameEntry.TABLE_NAME, new String[]{DatabaseSchema.UsernameEntry.PIANO_PATTERN_SUBMISSION},
                DatabaseSchema.UsernameEntry.USERNAME + " = ?", new String[]{username}, null, null, null);
        c.moveToFirst();
        boolean result = c.getInt(c.getColumnIndex(DatabaseSchema.UsernameEntry.PIANO_PATTERN_SUBMISSION)) == 1;
        c.close();
        return result;
    }

    //This method updates a users status for the color pattern
    private void setUserDoneWithPianoPattern() {
        DatabaseConnectionHelper db_connection = new DatabaseConnectionHelper(PianoPatternActivity.this);
        SQLiteDatabase db = db_connection.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(DatabaseSchema.UsernameEntry.PIANO_PATTERN_SUBMISSION, 1);
    ...
    private void lockOrUnlockPattern(String action) {
        for (int i = 0; i < 13; i++) {
            switch (action) {
                case "lock":
                    pianoButtonArray[i].setEnabled(false);
                    break;
                case "unlock":
                    pianoButtonArray[i].setEnabled(true);
                    break;
                default:
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    final String userFolder = users + userData.getUsername() + "/pianoPatternData/";
                    ...
                    for (int i = 0; i < userData.getRawPatterns().size(); i += 4) {
                        ...
                        String rawFileNameTemplate = "piano_" + userData.getUsername() + "_" + counter + "_raw";
                    ...
                    String metaFileNameTemplate = "piano_" + userData.getUsername() + "_metadata";
                    ...
                    for (int i = 0; i < touchData.size(); i++) {
                        ...
                        String touchFileNameTemplate = "piano_" + userData.getUsername() + "_" + counter + "_touch";
                    ...
                    for (int i = 0; i < userData.getPairedMeta().size(); i += 3) {
                        String pairFileNameTemplate = "piano_" + userData.getUsername() + "_" + counter + "_pairs";
        ...
        Thread t = new Thread() {
            ...
            public void run() {
                try {
                    ...
                    String[] webServiceRequests = {"PianoRawPatternUpload?", "PianoPatternMetadataUpload?", "PianoTouchDataUpload?", "PianoPairedMetadataUpload?"};
                    ...
                    for (int i = 0; i < 11; i++) {
                        String rawFileNameTemplate = "piano_" + userData.getUsername() + "_" + counter + "_raw.csv";
                    ...
                    String metaFileNameTemplate = "piano_" + userData.getUsername() + "_metadata.csv";
                    ...
                    counter = 1;
                    for (int i = 0; i < 11; i++) {
                        String touchFileNameTemplate = "piano_" + userData.getUsername() + "_" + counter + "_touch.csv";
                    ...
                    counter = 1;
                    for (int i = 0; i < 11; i++) {
                        String pairFileNameTemplate = "piano_" + userData.getUsername() + "_" + counter + "_pairs.csv";
    ...
    //Note: Indeed the authentication is done over HTTP and it is not secure, but this is not the point of the application, plus the documentation of RapidMiner Server on enabling HTTPS is very poor
